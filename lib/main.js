...
exports.main = function() {
  ...
  var calculateSlopeAndTrend = function(last_price, price, trend) {
    ...
    if (last_price == 0) {
      ...
      var st = price;
      var bt = price - last_price;
    }
    else {
      var st = .05* price + .95 * (trend[0] + trend[1]);
      var bt = .1 * (st - trend[0]) + .9 * trend[1];
    }
    var label_trend = '\u21d4'; // ⇔
      ...
      Request({
        ...
        onComplete: function (response) {
          ...
          if ((response != null) && (response.json != null)) {
            ...
            if ( price < 1 ) { // Allow more decimals for low price values
              round_factor = 10000;
            }
            ...
            //console.log(ticker.id + ": " + price + " -> " + ticker.tooltip
            //    + " (" + ticker.trend[0] + ", " + ticker.trend[1]+")");
