...
exports.main = function() {
  ...
  var registerEvents = function(tickerName, tickerCreator) {
    ...
    Preferences.on('p' + tickerName + 'Timer', function() { updateTickerCaller(tickerName, true); });
  };

  var updateStyleAllTickers = function() {
    for (var i in tickers) {
      if ( (tickers[i] != null) &&  (tickers[i].length >= 2)) {
        tickers[i][2](); // Call update Ticker method
      }
    }
  };
  ...
  var createTicker = function(id, label, currency, color, ticker_url, json_path) {
    ...
    var refresh_rate = getIntegerPreference("p" + id + "Timer"); // Construct preference refresh rate from ID
    if (refresh_rate < 1) {
    ...
    ticker.port.emit("updateStyle", color, font_size);
