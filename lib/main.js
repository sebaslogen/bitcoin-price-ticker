...
exports.main = function() {
  var createTicker = function(id, label, currency, color, ticker_url, json_path) {
    ...
    // Load and set ticker refresh rate from preferences
    var refresh_rate = 60;
    ...
    var ticker = new Widget({
      ...
      contentStyleFile: data.url("css/main.css"),
      contentScriptFile: [
    ...
    ticker.port.emit("update", color, font_size, default_content);
    ...
    function updateTicker() {
      ...
      Request({
        ...
        onComplete: function (response) {
          ...
          ticker.port.emit("update", color, font_size, new_content);
        }
      }).get();
    };
  ...
  var toggleTicker = function(tickerName, tickerCreator) {
    ...
    if ( enable ) { // Enable Ticker
      ...
      clearInterval(tickers[tickerName + 'Ticker'][2]); // Stop automatic refresh of removed ticker
