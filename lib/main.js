...
exports.main = function() {
  ...
  var updateTickerRefreshInterval = function() {
    ...
    for (var i in tickers) {
      if ( (tickers[i] != null) && (tickers[i].length >= 4)) {
        ...
        tickers[i][3] = setInterval(function() { tickers[i][1](); }, (refresh_rate * 1000));
      }
    }
  }
  ...
  var createTicker = function(id, label, currency, color, ticker_url, json_path) {
    ...
    if (currency == 'L') { // Litecoin can't have the gold Bitcoin background
    ...
    /*ticker.port.on("updateTickerWidth", function(new_width) {
      console.log("width recived:" + new_width);
      if ((new_width != null) && (new_width > 0)) {
        if (currency == "元") { // Yuan character needs extra pixels to be painted
          new_width += 8;
          console.log("adding extra 8px");
        }
        var default_ticker_width = getIntegerPreference("defaultTickerSpacing");
        if (default_ticker_width <= 0) {
          default_ticker_width = DEFAULT_TICKER_SPACING;
        }
        new_width += default_ticker_width;
        ticker.width = new_width;
      }
    });*/
      ...
      Request({
        ...
        onComplete: function (response) {
          ...
          if ((response != null) && (response.json != null)) {
            ...
            if (currency == 'L') { // Allow more decimals for Litecoin
              round_factor = 10000;
            }
  ...
  // Litecoin prices //
  var createBTCeLitecoinTicker = function() { return createTicker('BTCeLitecoin', 'BTCe', 'L', '#013ADF', "https://btc-e.com/api/2/ltc_btc/ticker", ['ticker','last']); };
  registerEvents('BTCeLitecoin', createBTCeLitecoinTicker);
  tickers['BTCeLitecoinTicker'] = createBTCeLitecoinTicker();

  var createVircurexLitecoinTicker = function() { return createTicker('VircurexLitecoin', 'Vircurex', 'L', '#0B0B3B', "https://vircurex.com/api/get_last_trade.json?base=LTC&alt=BTC", ['value']); };
  registerEvents('VircurexLitecoin', createVircurexLitecoinTicker);
  tickers['VircurexLitecoinTicker'] = createVircurexLitecoinTicker();

  var createKrakenLitecoinTicker = function() { return createTicker('KrakenLitecoin', 'Kraken', 'L', '#000066', "https://api.kraken.com/0/public/Ticker?pair=XBTLTC", ['result','XXBTXLTC','c',0]); };
